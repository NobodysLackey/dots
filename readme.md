...
- `querySelectorAll()` will output an *array* of all elements with that same class. 
...
- This can be accomplished in a similar fashion to the variables in Level 2

### Functionality
Since we have balls of *different* sizes in Level 3 that have *different* values, how do we adjust for that in our logic?
- First, we'll need to add a `click` event listener for each ball with the `js-ball` class
- Next, when a ball is clicked, we'll need to increment the score by each ball's `increment` value within it's `dataset` property
  - One way to accomplish this is:
    - <details><summary>HINT</summary><p>
      ...
      ```
      Notice that the `dataset` property object also has an `increment` key within it.

      What type of *notation* could be used to get the point value of the ball?
      </p>
      </details>
  - Now that we've accessed the increment value of each ball, we'll need to set it to a variable so we'll have access to it in our event listener callback function. We also need to make sure that the value is actually an integer value, since this is not guaranteed.
